...
**地铁网络覆盖模型**：由各线路站点的顺序构建图，相邻站间行驶时间按直线距离/均速估计（约 32 km/h，单段≥2 分钟），换乘另计约 3 分钟；用 Dijkstra 计算从某站到全网各站的站至站最小时间，得到时间预算内的「站至站可达」覆盖。该模型用于「纯地铁」模式，能覆盖城际/跨市线路，最长 180 分钟（不受高德等时圈 60 分钟限制）。
